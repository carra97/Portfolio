 ...
 * - `status: 'archived'` ⇒ el proyecto NO se enlaza. Está en el tipo, no en la memoria.
 * - La nota de prensa vive DENTRO de su case study (DISENO.md §7.4): una sola nota no
        ...
        return (
          <li key={project.name}>
            ...
            <div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <h3 className="h3">
                {linkable ? (
                  <a className="prose-link" href={project.url} rel="noopener noreferrer" target="_blank">
                    {project.name}
                  </a>
                ) : (
                  project.name
            ...
            {project.result && <p className="measure mt-3 text-pretty text-heading">{project.result}</p>}
            {project.status === 'archived' && <p className="small mt-3">{archivedLabel}</p>}
            ...
            {project.press && (
              <div className="mt-8 rounded-xl border border-line bg-surface p-6">
                ...
                <p className="measure mt-3 text-pretty">
                  La nota oficial del organismo me identifica como{' '}
                  <em className="text-heading">“{project.press.quote}”</em>.
                </p>
                <a
            ...
            {project.slug && (
              <p className="mt-6">
                <a className="prose-link" href={`/${lang}/proyectos/${project.slug}`}>
                  {caseStudyLabel} →
                </a>
              </p>
            )}
